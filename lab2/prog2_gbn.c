...
void A_output(struct msg message)
{
  ...
  if (seqnums_A - base_A >= MAX_BUFFER) {
    ...
    if (seqnums_A - base_A < WINDOW_SIZE) {
        ...
        printf("putting packet number %d into buffer since outside the window size of %d\n", seqnums_A-1, WINDOW_SIZE);
    }
  ...
  if (checksum == get_checksum(packet.seqnum, packet.acknum, packet.payload)) {
    if (packet.acknum != -1000)
    {
        ...
        if (base_A < packet.acknum + 1) {
            ...
            if (base_A < seqnums_A) {
                starttimer(0, timeout_value);
                for (int i= packet_to_sent; i < base_A + WINDOW_SIZE && i < seqnums_A; i++) {
                    log_packet('A', queue[i]);
                    tolayer3(0, queue[i]);
                    from_transport_A++;
                }
            }
...
void A_timerinterrupt()
{
  ...
  starttimer(0, timeout_value);
  for (int i= base_A; i < base_A + WINDOW_SIZE && i < seqnums_A; i++) {
    log_packet('A', queue[i]);
    tolayer3(0, queue[i]);
    from_transport_A++;
  }
...
void B_input(struct pkt packet)
{
  ...
  if (checksum == get_checksum(packet.seqnum, packet.acknum, packet.payload)) 
  {
    if (packet.acknum != -1000)
    {
        printf("something wrong acknum should be -1000 since unidirectional\n");
    } 
    else 
    {  
        ...
        {
            printf("not received expected packet from A, expected %d, got %d\n", last_received_B + 1, packet.seqnum);
        }
    }
  }
  else {
  ...
  struct pkt ack_packet = create_pkt(-1000, ack_nack_number, NULL);
  printf("ack/nack sent from B with acknum %d\n", ack_packet.acknum);
